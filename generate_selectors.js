...
const outputPath = 'data/dataset_with_selector.jsonl';
...
async function generateSelectors() {
  ...
  for await (const line of rl) {
    ...
    try {
      ...
      if (!first) continue;
      const selector = getCssSelector(first);
      output.write(JSON.stringify({ html, selector }) + '\n');
      count++;
    } catch (err) {
  ...
  output.end(() => {
    console.log(`Total pairs generated: ${count}`);
  });
}
